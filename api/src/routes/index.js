...
router.post('/pokemons', async(req, res) => {
        const {ID, Nombre, Imagen, Vida, Ataque, Defensa, Velocidad, Altura, Peso, Tipo} = req.body;         
        ...
        const respuesta = await postPokemons(ID, Nombre, Imagen, Vida, Ataque, Defensa, Velocidad, Altura, Peso, Tipo)  
        if (!respuesta.error)
        return res.status(200).json(respuesta)
        return res.status(503).json(respuesta)
})
